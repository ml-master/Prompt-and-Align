...
def train_model(args, x_train, x_test, y_train, y_test, tokenizer, max_len, n_epochs, batch_size, datasetname, iter, model, optimizer, scheduler):
...
# Initialize the model, optimizer, and scheduler once before the iterations
...
for iter in range(iterations):
    ...
    print(['Mean Test Accuracy:', np.mean(test_accs),
           'Mean Precision:', np.mean(prec_all),
           'Mean Recall:', np.mean(rec_all),
           'Mean F1:', np.mean(f1_all)])
